...
def find_dataset(database_name, species, **url_kwargs):
    ...
    database = databases[
        (databases["name"].isin([alt_name, database_name]))
...
def _fetch_dataset(
    dataset_name=None, database_name=None, species_name=None, **url_kwargs
):
    """
...
def fetch_data(
    ...
    **url_kwargs,
    ...
    if (hom_species is not None) and (hom_query is not None):
        attributes = attributes + dataset.extract_homology_attributes(
            hom_species, hom_query
        )
